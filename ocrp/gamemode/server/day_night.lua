...
local function ADD_SKY_KEYVALUE(name, default, mult_by_world_light) -- CHECKED, NOPE
	if CLIENT then
		P["sky_" .. name] = default
	end
	if SERVER then
...
if SERVER then
	hook.Add("InitPostEntity", "OCRP_Fog_Values", function()
		for k,v in pairs(ents.FindByClass("env_fog_controller")) do
			v:Fire("SetStartDist", "10")
			v:Fire("SetEndDist", "10")
		end
	end)
	...
	P.sun_angles = function(val)	
		...
		if ent:IsValid() then
			timer.Create("tod_sun_angles_hack", 0, 1, function()
				ent:SetAngles(val)
	...
	-- fuck dynamic shadows they suck
	...
	-- RealDope
	--[[P.fog_color = function(val)
		for k,v in pairs(ents.FindByClass("env_fog_controller")) do
			v:SetKeyValue("fogcolor", math.Round(val.x).." "..math.Round(val.y).." "..math.Round(val.z))
		end
	end
	P.fog_max_density = function(val)
		for k,v in pairs(ents.FindByClass("env_fog_controller")) do
			v:SetKeyValue("fogmaxdensity", tostring(val))
		end
	end]]
end
...
if CLIENT then	
	...
	timer.Create("tod_update_lightmap", 1, 0, function()
...
function tod.GetWeatherChance(probability)
	...
	probability = 0 --probability or 0.5
...
hook.Add("Think", "tod", function()
	...
	elseif tod.mode == 1 then -- realtime
		...
		local fraction = (H*3600 + M*60 + S) / 60
		time = fraction%1
	end
...
include("default_cycle.lua")

if SERVER then
	AddCSLuaFile("default_cycle.lua")
end
